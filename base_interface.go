...
func requestNew(options options) rest.Request {
	requestHeaders := map[string]string{
		...
		"User-Agent":    "cocoonmail/" + Version + ";go",
...
// API sets up the request to the Cocoonmail API, this is main interface.
...
// MakeRequest attempts a Cocoonmail request synchronously.
func MakeRequest(request rest.Request) (*rest.Response, error) {
	return MakeRequestWithContext(context.Background(), request)
}

// MakeRequestWithContext attempts a Cocoonmail request synchronously with context.Context.
